...
int Grafo::gulosoRandomizado(float alfa, int numInter, double tempo[], int seed){
    ...
    while (it<numInter) {  
        ...
        int peso0 = this->arestasOrdenadas[0]->getPeso();
        int faixa = peso0*(1+alfa);//calcula faixa de selecao
        int range = 0; //indice das arestas que se encaixam na faixa
        for (int i = 0; i < this->arestasOrdenadas.size(); i++){
            if(this->arestasOrdenadas[i]->getPeso()<=faixa)
                range ++;
            else
                break;
        }
        ...
        auto amostraIni = this->arestasOrdenadas.begin() + range+1;
        int amCont = maxAm;
        ...
        while(cont < this->ordem && copia.size() > 0){ 
            ...
            int auxPeso = copia[0]->getPeso();
            if(auxPeso > peso0) {
                //cout << "faixa: " << faixa << "auxF: " << auxF << endl;
                peso0 = auxPeso;
                faixa = peso0*(1+alfa);
                int copiaCont = 0;
                for (auto i = amostraIni; i != this->arestasOrdenadas.end(); i++){
                    Arestas* aux = *i;
                    if(aux->getPeso()<=faixa){
                        range ++;
                        copia.push_back(aux);
                        copiaCont ++;
                    }
