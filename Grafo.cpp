...
void Grafo::gulosoRandomizado(float alfa, int numInter){
    ...
    mid = clock();//salva o tempo depois da ordenaçao
    Agm* solucao = new Agm;//declara a agm soluçao
    Agm* melhorSolucao = new Agm;//declara a agm soluçao
    float faixa = arestasOrdenadas[0]->getPeso()*alfa;//obtendo o valor final possivel pra ser escolhido
    int k,i;//contador de indice
    while (i<numInter)
    {   
        int conexo[ordem];
        for (int i = 0; i < ordem; i++) // ao contrario
        ...
        solucao = nullptr;
        while(arestasOrdenadas.size() != 0){   
            if(cont == this->ordem)
                    break;
            k=rand()%arestasOrdenadas.size();
        ...
        if(melhorSolucao->ehVazia())
        {
            melhorSolucao = solucao;
        }
        else
        {
            if(solucao->calculaPesoTotal() < melhorSolucao->calculaPesoTotal())
            {
                melhorSolucao = solucao;
            }
        }
